...
        
        const itemWarning = itemPriceCalc.warning ? `<br><small style="color: var(--warning-color); font-weight: 600;">${itemPriceCalc.warning}</small>` : '';
        
        // AFFICHAGE DES DATES MODIFIABLES
        const datesDisplay = itemPriceCalc.isDaily ? `
            <div class="cart-dates-control">
                <label for="start-date-${item.id}">Début (${itemPriceCalc.multiplier}j):</label>
                <input type="date" id="start-date-${item.id}" value="${item.startDate}" 
                    onchange="updateCartDates(${item.id}, this.value, document.getElementById('end-date-${item.id}').value)">
                <label for="end-date-${item.id}">Fin:</label>
                <input type="date" id="end-date-${item.id}" value="${item.endDate}"
                    onchange="updateCartDates(${item.id}, document.getElementById('start-date-${item.id}').value, this.value)">
            </div>
        ` : `<p>Période: Sans impact sur le prix unitaire</p>`;
...
function updateCartDates(itemId, newStartDate, newEndDate) {
    ...
    if (item) {
        // Validation basique des dates (la logique de calcul gère l'affichage de l'erreur)
        if (newStartDate && newEndDate && (new Date(newStartDate) > new Date(newEndDate))) {
            showToast("Erreur: La date de début ne peut pas être postérieure à la date de fin. Veuillez corriger.");
        }
        ...
        saveCart();
        renderCart(); // Re-rendu pour mettre à jour le prix et le nombre de jours
    }
}
...
    }
    ...
    // Validation de l'adresse si la livraison est demandée
    if (isDelivery) {
        if (!deliveryAddressInput.value.trim()) {
            showToast("⚠️ Vous avez coché la livraison, veuillez renseigner l'adresse complète.");
...
    const deliveryAddress = isDelivery ? deliveryAddressInput.value.trim() : 'Non demandée (Retrait sur place)';
...
         if (container) {
             // Masquer le carrousel si vide pour ne pas laisser un bloc vide
...
             loadingMessage.style.color = 'var(--warning-color)'; 
        }
    }
}
...
        listContainer.innerHTML = '<p style="text-align: center; color: var(--text-light); padding: 30px;">Aucun produit trouvé correspondant à votre recherche.</p>';
        return;
    }
...
                ${cautionDisplay}
                <button class="button primary-button" onclick="showProductDetails('${product.id}')">Détails / Réserver</button>
            </div>
        `;
